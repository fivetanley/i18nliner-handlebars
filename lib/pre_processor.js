...
var PreProcessor = {
  ...
  inferParts: function(statements) {
    var defaultValue
      ...
      , tempMap
      , ret;
    ret = this.extractTempPlaceholders(statements);
    defaultValue = ret[0];
    tempMap = ret[1];
  ...
  stringForStatement: function(statement, tempMap) {
    ...
    switch (statement.type) {
      ...
      case 'mustache':
        key = "__i18nliner_" + tempMap.size + "__";
        tempMap.map[key] = statement.sexpr.id; // TODO: support sub-expressions
        tempMap.size += 1;
        return key;
    }
  },
  ...
  extractTempPlaceholders: function(statements) {
    var defaultValue = ''
      , i
      , statementsLen = statements.length
      , tempMap = {map: {}, size: 0};
